...
class SmoothedValue(object):
    ...
    def __init__(self, window_size=cfg.record_iter):
...
class Recorder(object):
    def __init__(self, cfg):
        ...
        log_dir = os.path.join(cfg.log_dir, 'log')
    ...
    def __str__(self):
        ...
        recording_state = '  '.join(
            ['epoch: {}', 'step: {}', 'lr: {:.6f}', '{}', 'data: {:.4f}', 'batch: {:.4f}', 'epoch: {:.4f}'])
        return recording_state.format(self.epoch, self.step, self.lr, loss_state, self.data_time, self.batch_time,
                                      self.epoch_time)
