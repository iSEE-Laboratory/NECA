...
class Network(nn.Module):
    ...
    def backward_deform(self, pts, pose_dirs, skinning_weight, bary_coords, vert_ids,rel, batch, scale):
        ...
        bary_coords, vert_ids, rel, fnormal = net_util.get_bary_coord(init_bigpose, batch['shaped_tvertices'], batch['faces'])
        ...
        return cnl_pts[None], tpose_dirs, skinning_weight
    ...
    def get_pose_feat(self, pts, batch):
        ...
        if cfg.concat_pose:
            ...
            # pose = batch['poses'][:, None,3:].repeat(1, pts.shape[1], 1)
    ...
    def forward(self, ray_o, ray_d, near, far, batch):
        ...
        albedo_latent, local_coords = self.get_latent(pose_pts, bary_coords, vert_ids, rel, fnormal, batch, scale)

        # transform points from the pose space to the cnl space
        cnl_pts, viewdir, skinning_weight = \
            self.backward_deform(pose_pts, pose_dirs, None, bary_coords, vert_ids,rel, batch, scale)
        ...
        sdf = 10 * torch.ones([n_batch, n_pixel * n_sample, 1]).to(cnl_pts)
        sdf[pind] = ret['sdf']
        ...
        light = self.light(None, pose_pts, grad_normal, None) / torch.pi
